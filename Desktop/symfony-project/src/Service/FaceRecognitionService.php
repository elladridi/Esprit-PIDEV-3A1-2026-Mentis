...
class FaceRecognitionService
{
    ...
    public function registerSample(int $userId, UploadedFile $imageFile): array
    {
        ...
        if (($detection['quality'] ?? 0) < self::QUALITY_THRESHOLD) {
            ...
            if (is_string($processedPath) && file_exists($processedPath)) {
                @unlink($processedPath);
            }

            return [
                'success' => false,
                'message' => 'Image too blurry (quality: ' . round((float) ($detection['quality'] ?? 0), 1) . ').',
            ];
        }
        ...
        return [
            ...
            'completed' => $completed,
            'quality' => round((float) ($detection['quality'] ?? 0), 1),
            'message' => $completed
    ...
    public function verifyAgainstAll(array $users, UploadedFile $imageFile): array
    {
        ...
        $verifyFacePath = $detection['processed_path'] ?? null;

        if (!is_string($verifyFacePath)) {
            return ['success' => false, 'message' => 'Processed face not found.'];
        }
        ...
        foreach ($users as $user) {
            ...
            $samples = $this->getUserSamples($userId);
            $scores = [];
        ...
        $matched = ($bestScore >= self::SIMILARITY_THRESHOLD) && ($bestUser instanceof User);
    ...
    private function runDetect(string $imagePath): array
    {
        ...
        $lastLine = end($lines);
        $result = is_string($lastLine) ? json_decode($lastLine, true) : null;
