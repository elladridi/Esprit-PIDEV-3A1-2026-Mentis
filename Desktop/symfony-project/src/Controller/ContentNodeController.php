...
public function index(Request $request, ContentNodeRepository $contentNodeRepository, UserRepository $userRepository, BookRecommendationService $bookService): Response
{
    /** @var User $user */
    $user = $this->getUser();
    if (!$user) {
    ...
    // Grouping nodes into categories (you could extend this if needed)
    // For now, they all go into the library section as requested.
    ...
    public function saveGenerated(Request $request, EntityManagerInterface $em): JsonResponse
    {
        /** @var User $user */
        $user = $this->getUser();
        ...
        // Convert sections to a formatted description
        $description = "";
        ...
        $contentNode->setAssignedUsers([]); // Initially empty
